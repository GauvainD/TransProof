    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first && !edge(*w_iter.first, *u_iter.first, g).second)
                    {
                        phoeg::Graph h(g);
                        remove_edge(*u_iter.first, *v_iter.first, h);
                        add_edge(*u_iter.first, *w_iter.first, h);
                        std::string desc = str(boost::format("remove (%d;%d) and add (%d;%d)") % *u_iter.first % *v_iter.first % *u_iter.first % *w_iter.first);
    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                phoeg::Graph h(g);
                remove_edge(*u_iter.first, *v_iter.first, h);
                std::string desc = str(boost::format("deleted edge between %ld and %ld") % *u_iter.first % *v_iter.first);
                struct transformation_result res = {h, desc};
                m.push_back(res);
            }
        }
...
std::vector<struct transformation_result> addEdge(const phoeg::Graph& g)
{
    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            ...
            {
                phoeg::Graph h(g);
                add_edge(*u_iter.first, *v_iter.first, h);
                std::string desc = str(boost::format("added edge between %ld and %ld") % *u_iter.first % *v_iter.first);
                struct transformation_result res = {h, desc};
                m.push_back(res);
            }
        }
    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first && !edge(*w_iter.first, *u_iter.first, g).second && edge(*w_iter.first, *v_iter.first, g).second)
                    {
                        phoeg::Graph h(g);
                        remove_edge(*u_iter.first, *v_iter.first, h);
                        add_edge(*u_iter.first, *w_iter.first, h);
    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first)
                    {
                        std::pair<phoeg::vertex_iter, phoeg::vertex_iter> x_iter = vertices(g);
                        for (; x_iter.first != x_iter.second; x_iter.first++)
                        {
                            if (x_iter.first != w_iter.first && !edge(*x_iter.first, *w_iter.first, g).second)
                            {
                                phoeg::Graph h(g);
                                remove_edge(*u_iter.first, *v_iter.first, h);
                                add_edge(*w_iter.first, *x_iter.first, h);
                                std::string desc = str(boost::format("remove (%d;%d) and add (%d;%d)") % *u_iter.first % *v_iter.first % *w_iter.first % *x_iter.first);
                                struct transformation_result res = {h, desc};
                                results.push_back(res);
                            }
                        }
            ...
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first)
                    {
                        ...
                        for (; x_iter.first != x_iter.second; x_iter.first++)
                        {
                            if (x_iter.first != w_iter.first && x_iter.first != v_iter.first && x_iter.first != u_iter.first && !edge(*x_iter.first, *w_iter.first, g).second)
                            {
                                phoeg::Graph h(g);
                                remove_edge(*u_iter.first, *v_iter.first, h);
                                add_edge(*w_iter.first, *x_iter.first, h);
    ...
    for (; u_iter.first != u_iter.second; u_iter.first++)
    {
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first && !edge(*w_iter.first, *u_iter.first, g).second && !edge(*v_iter.first, *w_iter.first, g).second)
                    {
                        phoeg::Graph h(g);
                        remove_edge(*u_iter.first, *v_iter.first, h);
                        add_edge(*u_iter.first, *w_iter.first, h);
                        add_edge(*v_iter.first, *w_iter.first, h);
                        std::string desc = str(boost::format("remove (%d;%d) and add (%d;%d) and (%d;%d)") % *u_iter.first % *v_iter.first % *u_iter.first % *w_iter.first % *v_iter.first % *w_iter.first);
                        struct transformation_result res = {h, desc};
                        results.push_back(res);
                    }
                }
        ...
        for (; v_iter.first != v_iter.second; v_iter.first++)
        {
            if (u_iter.first != v_iter.first && !edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    ...
                    {
                        phoeg::Graph h(g);
                        remove_edge(*u_iter.first, *w_iter.first, h);
                        remove_edge(*v_iter.first, *w_iter.first, h);
                        add_edge(*u_iter.first, *v_iter.first, h);
                        std::string desc = str(boost::format("remove (%d;%d) and (%d;%d) and add (%d;%d)") % *u_iter.first % *w_iter.first % *v_iter.first % *w_iter.first % *u_iter.first % *v_iter.first);
                        struct transformation_result res = {h, desc};
                        results.push_back(res);
                    }
                }
            ...
            if (u_iter.first != v_iter.first && edge(*u_iter.first, *v_iter.first, g).second)
            {
                ...
                for (; w_iter.first != w_iter.second; w_iter.first++)
                {
                    if (w_iter.first != v_iter.first && w_iter.first != u_iter.first && !edge(*u_iter.first, *w_iter.first, g).second)
                    {
                        ...
                        for (; x_iter.first != x_iter.second; x_iter.first++)
                        {
                            if (x_iter.first != w_iter.first && !edge(*x_iter.first, *v_iter.first, g).second && !edge(*x_iter.first, *w_iter.first, g).second)
                            {
                                phoeg::Graph h(g);
                                remove_edge(*u_iter.first, *v_iter.first, h);
